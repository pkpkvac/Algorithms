...
class StockSpannerBruteForce:
    ...
    def next(self, price: int) -> int:
        days = 1
        end = len(self.stocks) - 1
        for i in range(end, -1, -1):
            if price >= self.stocks[i]:
                days += 1
            else:
                break
...
class StockSpanner:
    # The Stack Idea: Store "Barriers"
    # Instead of checking every previous day,
    # use a monotonic decreasing stack to store
    # only the days that could potentially "block" future spans.
    # Key Insight:
    # If today's price is higher than yesterday's,
    # then yesterday becomes "irrelevant" for future
    # calculations because today will always be a better "barrier."
    def __init__(self):
        self.stocks = [] 
    ...
    def next(self, price: int) -> int:
        ...
        while len(self.stocks) > 0:
            # Get the top element (last element in the list)
            top_index = len(self.stocks) - 1
            top_price, top_span = self.stocks[top_index]
            
            # If current price >= top price, pop it and accumulate span
            if price >= top_price:
                popped_price, popped_span = self.stocks.pop()
                span += popped_span  # Accumulate spans
            else:
                break  # Stop if current price < top price
...
# Your StockSpannerBruteForce object will be instantiated and called as such:
# stockSpanner = StockSpannerBruteForce()
