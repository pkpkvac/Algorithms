...
func TestDiameterOfBinaryTree(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := DiameterOfBinaryTreeRecursive(tt.root)
			if !reflect.DeepEqual(result, tt.expected) {
				t.Errorf("DiameterOfBinaryTreeRecursive() = %v, want %v", result, tt.expected)
			}
		})
	}
...
func BenchmarkDiameterOfBinaryTreeRecursive(b *testing.B) {
	// Use a reasonably large complete tree
	root := buildCompleteTree(2047) // height ~11
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = DiameterOfBinaryTreeRecursive(root)
	}
}

// func BenchmarkDiameterOfBinaryTreeIterative(b *testing.B) {
// 	root := buildCompleteTree(2047)
// 	b.ReportAllocs()
// 	b.ResetTimer()
// 	for i := 0; i < b.N; i++ {
// 		_ = DiameterOfBinaryTreeIterative(root)
// 	}
// }
